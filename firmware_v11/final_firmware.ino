...
void reconnect_mqtt() {
    if (!mqttClient.connected()) {
        ...
        if (mqttClient.connect("ESP32_WashingMachine", MQTT_USER, MQTT_PASSWORD)) {
            Serial.println("OK");
        } else {
...
#define ALARM_ENTER_THR   5
#define ALARM_EXIT_THR    1
...
void ai_task(void*) {
    ...
    while (true) {
        ...
        if (!in_alarm_state && high_count >= ALARM_ENTER_THR) {
            in_alarm_state = true;
        } else if (in_alarm_state && high_count <= ALARM_EXIT_THR) {
...
void setup() {
    Serial.begin(921600);
    delay(500);
    Serial.println("=== BOOT FINAL (True Parallel Dual-Core) ===");
