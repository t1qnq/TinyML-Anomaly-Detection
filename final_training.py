"""Huấn luyện Autoencoder ba pha không rò rỉ dữ liệu cho ESP32-S3 TinyML.

Script huấn luyện một Autoencoder riêng cho từng pha giặt:
  - GENTLE
  - STRONG
  - SPIN
...
def load_training_dependencies() -> None:
    """Import TensorFlow/TMOT tại thời điểm chạy để giảm chi phí import sớm."""
...
def set_reproducibility(seed: int) -> None:
    """Cố định seed cho NumPy/TensorFlow để split và augmentation lặp lại được."""
    np.random.seed(seed)
    tf.random.set_seed(seed)


def load_features(csv_path: str | Path) -> np.ndarray:
    """Đọc CSV feature 19 cột do `final_feature_extraction.py` sinh ra."""
...
def compute_phase_thresholds() -> dict[str, object]:
    """Trả về hai ngưỡng K-Means cố định dùng trong đồ án.

    Giá trị được cố định để tái lập, nhưng nguồn gốc vẫn là dữ liệu: K-Means
    được chạy trên phân phối `var_z` của 14.000 cửa sổ để tìm 3 cụm vận hành,
    sau đó lấy trung điểm giữa các tâm cụm đã sắp xếp làm ranh giới định tuyến.
    """
    return {
...
def split_by_phase(x_raw: np.ndarray, thr1: float, thr2: float) -> dict[str, np.ndarray]:
    """Chia các cửa sổ raw thành GENTLE, STRONG và SPIN theo `var_z`."""
    var_z = x_raw[:, 18]
    return {
...
def compute_vib_clip(x_raw: np.ndarray) -> dict[int, float]:
    """Tính ngưỡng clip rung để giảm ảnh hưởng của xung cảm biến lên scaler."""
    return {
...
def apply_vib_clip(x_raw: np.ndarray, clip: dict[int, float]) -> np.ndarray:
    """Áp dụng clip rung trong miền vật lý trước khi chia pha và huấn luyện."""
    out = x_raw.copy()
...
def safe_robust_fit(data: np.ndarray) -> RobustScaler:
    """Fit RobustScaler và chặn scale quá nhỏ để tránh chuẩn hóa mất ổn định."""
...
def scale_subset(raw: np.ndarray, vib_scaler: RobustScaler) -> np.ndarray:
    """Chuẩn hóa log-Mel và rung về miền [0, 1] trước khi đưa vào model."""
...
def augment_raw_data(x_raw: np.ndarray, target_size: int) -> np.ndarray:
    ...
    sigma=0.02 được cộng riêng sau khi scale về [0, 1] để huấn luyện denoising
    Autoencoder.
    """
    if len(x_raw) == 0:
...
def weighted_mae(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
    """Tính Weighted MAE, trong đó 6 đặc trưng rung có trọng số cao hơn."""
    weights = tf.constant(WEIGHTS, dtype=tf.float32)
    return tf.reduce_sum(tf.abs(y_true - y_pred) * weights, axis=-1) / WEIGHT_SUM


def build_model() -> tf.keras.Model:
    """Tạo kiến trúc Dense Autoencoder đối xứng dùng chung cho ba pha."""
...
def convert_to_int8(qat_model: tf.keras.Model, representative_data: np.ndarray) -> bytes:
    """Chuyển model QAT Keras sang TFLite INT8 đầy đủ để nhúng firmware."""
    def representative_gen():
        """Sinh mẫu normal đã scale để TFLite hiệu chỉnh thang lượng tử INT8."""
...
def run_tflite_mae(model_bytes: bytes, data: np.ndarray) -> np.ndarray:
    """Chạy suy luận TFLite INT8 và trả về Weighted MAE cho từng dòng dữ liệu."""
...
def c_array(data: bytes, name: str) -> str:
    """Định dạng buffer TFLite thành mảng C được căn hàng cho firmware."""
    rows = []
...
def c_floats(values: np.ndarray) -> str:
    """Định dạng mảng float với độ chính xác ổn định cho header C sinh ra."""
...
def write_model_header(
    ...
    """Ghi `model_data_final.h` gồm ngưỡng, scaler và ba mảng TFLite."""
    out = Path(path)
    out.parent.mkdir(parents=True, exist_ok=True)

    header = "#ifndef MODEL_DATA_FINAL_H\n#define MODEL_DATA_FINAL_H\n\n"
    header += "// Sinh tự động bởi final_training.py\n"
